...
void Timer::onTimer() {
  ...
  for (TimerEvent& event : tmps){
    const std::function<void()>&f = event.getCallBack();
    if (f)
      f();

    if (event.isRepeated() ) {
      event.resetArriveTime();
      addTimerEvent(std::move(event) ); 
    }
...
void Timer::resetTimer() {
  ...
  value.it_value.tv_sec  = m_arrive_time / 1000;
  value.it_value.tv_nsec = (m_arrive_time % 1000) * 1000000;
